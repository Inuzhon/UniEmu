...
Разрешена только директива `#load`. Директивы подключения сборок и NuGet-пакетов запрещены:
...
При сохранении или backend IntelliSense такие директивы возвращают диагностическую ошибку `CSX001`. Сохранение скрипта не применяет изменения, если в коде есть `#r`.

## Ограничения безопасности

Скрипт не может расширять набор доступных сборок через `#r`. Доступный API задается backend-окружением CSX и проектом `UniEmu.Scripting.Api`.
