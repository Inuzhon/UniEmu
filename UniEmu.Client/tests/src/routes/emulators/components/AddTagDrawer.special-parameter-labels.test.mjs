...
test('special parameter select renders human-readable labels', async () => {
  const componentSource = await readFile(join(join(dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', '..', 'src', 'routes', 'emulators', 'components'), 'tag-editor/TagBasicsSection.tsx'), 'utf8');
  const typesSource = await readFile(join(join(dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', '..', 'src', 'routes', 'emulators', 'components'), '../../../types/uniemu.ts'), 'utf8');
...
test('tag trigger select hides cron option from drawer', async () => {
  const componentSource = await readFile(join(join(dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', '..', 'src', 'routes', 'emulators', 'components'), 'tag-editor/TagTriggerSection.tsx'), 'utf8');
